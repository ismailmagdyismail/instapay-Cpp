...
std::optional<OperationResult> InMemoryUsers::addUser(const std::unique_ptr<IUser>& newUser) {
    const std::string userName = newUser->getUserProfile().userName;
    if( this->db.find(userName) != this->db.end()){
        return OperationResult{false, 401, "user already exists with this data"};
    }
